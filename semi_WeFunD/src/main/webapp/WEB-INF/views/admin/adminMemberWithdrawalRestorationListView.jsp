...
<body>
		<div class="adminall">
			...
			<div class="adminWrap">
											...
											<tr>
												...
												<td>
													<button class="btn-modal btn btn-dark" data-toggle="modal"
														data-target="#myModal" data-id="${m.userNo}">
														회원복구</button>
												</td>
											</tr>
										</c:forEach>
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-footer">
								<form
									action="restoration.mem"
									method="post">
									<input type="hidden" class="body-contents" id="userNo"
										name="userNo">
					...
					<c:choose>
						<c:when test="${empty keyword }">
							<c:if test="${pi.currentPage > 1}">
								<button
									onclick="location.href='adminWRSerchList.mem?cpage=${pi.currentPage - 1}'"
									class="btn  btnYellow">&lt;
								</button>
							</c:if>
							<c:forEach var="i" begin="${pi.startPage}" end="${pi.endPage}">
								<c:choose>
									<c:when test="${i ne pi.currentPage}">
										<button
											onclick="location.href='adminWRSerchList.mem?cpage=${i}'"
						...
						<c:otherwise>
							<c:if test="${pi.currentPage > 1}">
								<button
									onclick="location.href='adminWRSerchList.mem?
				...
				<form action="adminWRSerchList.mem">
					<div class="srch">
